...
- **강사(INSTRUCTOR) 행의 주민등록번호 노출**: `bizNumberEnc` 필드는 업체(VENDOR)에는 사업자등록번호를, 강사(INSTRUCTOR)에는 주민등록번호를 담는다(화면에도 `900101-1******` 형태로 마스킹되어 표시됨). "사업자번호" 컬럼은 이 필드를 그대로 복호화하므로, 강사 행에서는 13자리 주민등록번호가 원문으로 다운로드 파일에 포함된다. 이는 확인 후 의도된 동작으로 결정됨 — 정산담당자/관리자가 업무상 원문이 필요하다는 판단(2026-07-29). 별도의 마스킹 예외 처리를 하지 않는다.
